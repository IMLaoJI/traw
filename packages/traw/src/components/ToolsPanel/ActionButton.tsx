...
const isAllGroupedSelector = (s: TDSnapshot) => {
  ...
  return selectedShapes.every(
    (shape) =>
      ...
      (shape.parentId === selectedShapes[0].parentId && selectedShapes[0].parentId !== s.appState.currentPageId),
  );
};
      ...
      <DMContent sideOffset={16} side={contentSide}>
        <>
          <ButtonsRow>
            ...
            <Tooltip label={isAllLocked ? 'Unlock' : 'Lock'} kbd={`#L`} id="TD-Tools-Lock">
              <ToolButton disabled={!hasSelection} onClick={handleToggleLocked}>
                {isAllLocked ? <LockClosedIcon /> : <LockOpen1Icon />}
              </ToolButton>
            </Tooltip>
            <Tooltip label={isAllAspectLocked ? 'Unlock Aspect Ratio' : 'Lock Aspect Ratio'} id="TD-Tools-AspectRatio">
          ...
          <Divider />
          <ButtonsRow>
            <Tooltip label={'Align Left'} id="TD-Tools-AlignLeft">
              <ToolButton disabled={!hasTwoOrMore} onClick={alignLeft}>
                <AlignLeftIcon />
              </ToolButton>
            </Tooltip>
            <Tooltip label={'Align Center X'} id="TD-Tools-AlignCenterHorizontal">
              <ToolButton disabled={!hasTwoOrMore} onClick={alignCenterHorizontal}>
                <AlignCenterHorizontallyIcon />
              </ToolButton>
            </Tooltip>
            <Tooltip label={'Align Right'} id="TD-Tools-AlignRight">
              <ToolButton disabled={!hasTwoOrMore} onClick={alignRight}>
                <AlignRightIcon />
              </ToolButton>
            </Tooltip>
            <Tooltip label={'Stretch Horizontal'} id="TD-Tools-StretchHorizontal">
              <ToolButton disabled={!hasTwoOrMore} onClick={stretchHorizontally}>
                <StretchHorizontallyIcon />
              </ToolButton>
            </Tooltip>
            <Tooltip label={'Distribute Horizontal'} id="TD-Tools-SpaceEvenlyHorizontal">
          ...
          <ButtonsRow>
            <Tooltip label={'Align Top'} id="TD-Tools-AlignTop">
              <ToolButton disabled={!hasTwoOrMore} onClick={alignTop}>
                <AlignTopIcon />
              </ToolButton>
            </Tooltip>
            <Tooltip label={'Align Vertial Center'} id="TD-Tools-AlignCenterVertical">
              <ToolButton disabled={!hasTwoOrMore} onClick={alignCenterVertical}>
                <AlignCenterVerticallyIcon />
              </ToolButton>
            </Tooltip>
            <Tooltip label={'Align Bottom'} id="TD-Tools-AlignBottom">
              <ToolButton disabled={!hasTwoOrMore} onClick={alignBottom}>
                <AlignBottomIcon />
              </ToolButton>
            </Tooltip>
            <Tooltip label={'Stretch Vertical'} id="TD-Tools-StretchVertical">
              <ToolButton disabled={!hasTwoOrMore} onClick={stretchVertically}>
                <StretchVerticallyIcon />
              </ToolButton>
            </Tooltip>
            <Tooltip label={'Distribute Vertical'} id="TD-Tools-SpaceEvenlyVertical">
